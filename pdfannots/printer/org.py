...
class OrgPrinter(Printer):
    ...
    def print_file(
        ...
        if self.print_filename:
            ...
            # When not printing filename, just yield all body content
            yield from body_iter
    ...
    def format_bullet(
        ...
        """
        Format an Org bullet, wrapped as desired.
        """
        ...
        for para in paras[1:]:
            ...
            if inquote:
                ...
                # Add the paragraph content (wrapped or not)
                if self.wrap_column:
                    wrapper = textwrap.TextWrapper(width=self.wrap_column)
                    ret = ret + wrapper.fill(para)
                else:
                    ret = ret + para
                
                # End of quote block
                if npara == quotepos + quotelen - 1:
                    ret = ret + '\n#+end_quote'
            else:
                # Regular paragraph (not in quote)
                if self.wrap_column:
                    ret = ret + self.bullet_tw2.fill(para)
                else:
                    ret = ret + self.BULLET_INDENT2 + para
...
class GroupedOrgPrinter(OrgPrinter):
